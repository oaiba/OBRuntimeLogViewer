...
void UOBRuntimeLogViewerSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	//
	// OnPostLoadMapDelegateHandle = FCoreUObjectDelegates::PostLoadMapWithWorld.AddUObject(
	// 	this, &UOBRuntimeLogViewerSubsystem::OnWorldChanged);
	//
	...
	if (!LogViewerWidgetInstance)
	{
		...
		if (WidgetClass == nullptr)
		{
			UE_LOG(LogTemp, Error,
			       TEXT("LogViewerWidgetClass is not set in Project Settings -> Plugins -> Runtime Log Viewer!"));
			return;
		}
	...
	bool bShowLogs, const FString& FilterText)
{
	...
	for (const FOBLogMessage& Log : AllLogs)
	{
		...
		if (bShowLogs && (Log.Verbosity == EOBRuntimeLogVerbosity::Log || Log.Verbosity ==
			EOBRuntimeLogVerbosity::Display))
			bVerbosityMatch = true;
...
void UOBRuntimeLogViewerSubsystem::OnWorldChanged(UWorld* World)
{
	if (World && World->IsGameWorld())
	{
		...
		{
			FTimerHandle DummyHandle;
			World->GetTimerManager().SetTimer(DummyHandle, this, &UOBRuntimeLogViewerSubsystem::ShowLogViewer, 0.1f,
			                                  false);
		}
	}
}
